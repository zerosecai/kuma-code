...
- Author package entrypoint is `@kuma-code/plugin/tui`.
...
- Import types from `@kuma-code/plugin/tui`.
- `@kuma-code/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@kuma-code/plugin/tui"
...
When `plugin` entries exist in a writable `.opencode` dir or `KILO_CONFIG_DIR`, OpenCode installs `@kuma-code/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@kuma-code/plugin/tui`.
