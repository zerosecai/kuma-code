...
The `search_files` tool performs regex searches across multiple files in your project. It helps Kuma Code locate specific code patterns, text, or other content throughout your codebase with contextual results.
...
- When Kuma Code needs to find where specific functions or variables are used
- When Kuma Code helps with refactoring and needs to understand usage patterns
- When Kuma Code needs to locate all instances of a particular code pattern
- When Kuma Code searches for text across multiple files with filtering capabilities
...
- When asked to refactor a function, Kuma Code first searches for all places the function is used to ensure comprehensive changes.
- When investigating bugs, Kuma Code searches for similar patterns to identify related issues across the codebase.
- When addressing technical debt, Kuma Code locates all TODO comments across the project.
- When analyzing dependencies, Kuma Code finds all imports of a particular module.
