...
description: "Integrate Kuma Code with your shell environment"
---

# Terminal Shell Integration

Terminal Shell Integration is a key feature that enables Kuma Code to execute commands in your terminal and intelligently process their output. This bidirectional communication between the AI and your development environment unlocks powerful automation capabilities.
...
Shell integration is automatically enabled in Kuma Code and connects directly to your terminal's command execution lifecycle without requiring any setup from you. This built-in feature allows Kuma Code to:
...
When Kuma Code needs to perform tasks like installing dependencies, starting a development server, or analyzing build errors, shell integration works behind the scenes to make these interactions smooth and effective.

## Getting Started with Shell Integration

Shell integration is built into Kuma Code and works automatically in most cases. If you see "Shell Integration Unavailable" messages or experience issues with command execution, try these solutions:
...
Kuma Code provides several settings to fine-tune shell integration. Access these in the Kuma Code panel under Settings → Terminal.
...
Adds a small pause after running commands to help Kuma Code capture all output correctly. This setting can significantly impact shell integration reliability due to VSCode's implementation of terminal integration across different operating systems and shell configurations:
...
Helps PowerShell run the same command multiple times in a row. Enable this if you notice Kuma Code can't run identical commands consecutively in PowerShell.
...
Prevents ZSH from adding special characters at the end of output lines that can confuse Kuma Code when reading terminal results.
...
Makes Kuma Code work better with the popular [Oh My Zsh](https://ohmyz.sh/) shell customization framework. Turn this on if you use Oh My Zsh and experience terminal issues.
...
Improves compatibility if you use the Powerlevel10k theme for ZSH. Turn this on if your fancy terminal prompt causes issues with Kuma Code.
...
Helps Kuma Code work with custom ZSH configurations without interfering with your personal shell settings and customizations.
...
**Alternative**: Enable the Powerlevel10k Integration setting in Kuma Code.
...
1. **Set Terminal Command Delay**:
   - Set the Terminal Command Delay to 50ms in Kuma Code settings
...
4. **ZSH Users**:
   - Try enabling some or all ZSH-related workarounds in Kuma Code settings
...
**Issue**: If text is already typed in the terminal when Kuma Code tries to run a command, Kuma Code will press Ctrl+C first to clear the line, which can interrupt running processes.

**Workaround**: Make sure your terminal prompt is empty (no partial commands typed) before asking Kuma Code to execute terminal commands.

### Multi-line Command Issues

**Issue**: Commands that span multiple lines can confuse Kuma Code and may show output from previous commands mixed in with current output.
...
1. **Premature Completion**: PowerShell sometimes tells Kuma Code a command is finished before all the output has been shown.
...
When using the Kuma Code VS Code extension with the Agent Manager, each agent session gets its own dedicated VS Code terminal.
...
1. Check the [Kuma Code GitHub Issues](https://github.com/Kilo-Org/kuma-code/issues) to see if others have reported similar problems
