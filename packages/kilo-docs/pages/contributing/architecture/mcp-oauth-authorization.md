...
Many MCP servers require authentication to access protected resources. Currently, Kuma Code only supports static credential configuration (API keys, tokens) which must be manually entered and stored. This creates friction for users and security concerns for enterprises.

The MCP specification defines an OAuth 2.1-based authorization flow that enables secure, user-friendly authentication without requiring users to manually manage credentials. This document specifies how Kuma Code will implement the MCP Authorization specification to support OAuth-enabled MCP servers.
...
- **MCP Client** (Kuma Code) - Acts as OAuth 2.1 Client, obtains tokens on behalf of users
...
For Client ID Metadata Documents, Kuma Code needs to host a metadata document. We will use static hosting on kuma-code.ai:

- Host at `https://kuma-code.ai/.well-known/oauth-client/vscode-extension.json`
...
```json
{
  "client_id": "https://kuma-code.ai/.well-known/oauth-client/vscode-extension.json",
  "client_name": "Kuma Code",
  "client_uri": "https://kuma-code.ai",
  "logo_uri": "https://kuma-code.ai/logo.png",
  "redirect_uris": ["http://127.0.0.1:0/callback", "vscode://kuma-code.kilo-code/oauth/callback"],
...
2. **VS Code URI Handler** (Fallback)
   - Register `vscode://kuma-code.kilo-code/oauth/callback` URI handler
...
- [ ] Host Kuma Code client metadata at kuma-code.ai
