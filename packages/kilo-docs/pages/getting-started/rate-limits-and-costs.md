...
Understanding and managing API usage is crucial for a smooth and cost-effective experience with Kuma Code. This section explains how to track your token usage, costs, and how to configure rate limits.

## Token Usage

Kuma Code interacts with AI models using tokens. Tokens are essentially pieces of words. The number of tokens used in a request and response affects both the processing time and the cost.
...
Kuma Code automatically calculates the estimated cost of each API request based on the configured model's pricing. This cost is displayed in the chat history, next to the token usage.
...
To prevent accidental overuse of the API and to help you manage costs, Kuma Code allows you to set a rate limit. The rate limit specifies the minimum time (in seconds) between API requests.

**How to configure:**

1.  Open the Kuma Code settings ({% codicon name="gear" /%} icon in the top right corner).
...
If you set the rate limit to 10 seconds, Kuma Code will wait at least 10 seconds after one API request completes before sending the next one.
...
- **Use Custom Instructions:** Provide custom instructions to guide Kuma Code's behavior and reduce the need for lengthy explanations in each prompt.
...
By understanding and managing your API usage, you can use Kuma Code effectively and efficiently.
