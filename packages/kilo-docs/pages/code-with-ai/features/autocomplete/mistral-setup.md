...
This guide walks you through setting up Mistral's Codestral model for free autocomplete in Kuma Code. Mistral offers a free tier that's perfect for getting started with AI-powered code completions.
...
- A [Kuma Code account](https://app.kilo.ai) (free to create)
...
Once your BYOK key is saved, Kuma Code's autocomplete will automatically use your Codestral key through the Kilo Gateway. No additional configuration is needed in the extension.

1. Open VS Code with the Kuma Code extension installed.
...
- **Autocomplete not appearing?** Check that autocomplete is enabled in Kuma Code settings (it is on by default). Also verify you are signed into Kuma Code in the extension.
...
{% youtube url="https://www.youtube.com/embed/0aqBbB8fPho" caption="Setting up Mistral for free autocomplete in Kuma Code" /%}

## Step 1: Open Kuma Code Settings

In VS Code, open the Kuma Code panel and click the **Settings** icon (gear) in the top-right corner.

![Open Kuma Code Settings](/docs/img/mistral-setup/01-open-kilo-code-settings.png)
...
{% callout type="note" %}
When creating an autocomplete profile, you don't need to select a specific model—Kuma Code will automatically use the appropriate Codestral model optimized for code completions.
{% /callout %}
...
## Step 9: Paste API Key in Kuma Code

Return to Kuma Code settings and paste your API key into the **Mistral API Key** field.
