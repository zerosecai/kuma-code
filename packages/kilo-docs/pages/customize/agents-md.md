...
The Kuma Code **memory bank** feature has been deprecated in favor of AGENTS.md.
...
1. Examine the contents in `.kuma-code/rules/memory-bank/`
2. Move that content into your project's `AGENTS.md` file (or ask Kilo to do it for you)
   {% /callout %}

## What is AGENTS.md?

AGENTS.md is an open standard for configuring AI agent behavior in software projects. It's a simple Markdown file placed at the root of your project that contains instructions for AI coding assistants. The standard is supported by multiple AI coding tools, including Kuma Code, Cursor, and Windsurf.
...
When working in a subdirectory, Kuma Code will load both the root AGENTS.md and any subdirectory AGENTS.md files, with subdirectory files taking precedence for conflicting instructions.

## File Protection

Both `AGENTS.md` and `AGENT.md` are **write-protected files** in Kuma Code. This means:
...
## How AGENTS.md Works in Kuma Code

### Loading Behavior

When you start a task in Kuma Code:

1. Kuma Code checks for `AGENTS.md` or `AGENT.md` at the project root
...
AGENTS.md works alongside Kuma Code's other configuration systems:

| Feature | Scope | Location | Purpose | Priority |
|---|---|---|---|---|
| **[Mode-specific Custom Rules](/docs/customize/custom-rules)** | Project | `.kuma-code/rules-{mode}/` | Mode-specific rules and constraints | 1 (Highest) |
| **[Custom Rules](/docs/customize/custom-rules)** | Project | `.kuma-code/rules/` | Kuma Code-specific rules and constraints | 2 |
| **[AGENTS.md](/docs/customize/agents-md)** | Project | `AGENTS.md` | Universal standard for any AI coding tool | 3 |
| **[Global Custom Rules](/docs/customize/custom-rules)** | Global | `~/.kuma-code/rules/` | Global Kuma Code rules | 4 |
...
- **[Custom Rules](/docs/customize/custom-rules)** - Kuma Code-specific rules with more control
